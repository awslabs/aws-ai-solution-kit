...
在对图片进行文字识别之前，您需要通过**增加模版**API创建模板。为了提高文字识别准确性，请选择文字内容清晰并与被识别图像细节近似的图片创建模版。创建模版时每个待识别区域都需要指定矩形框的四个坐标点与该区域的名称，同一个模版可以包含多个识别区域。您可以使用常见的图像处理软件如 [GIMP](https://www.gimp.org/downloads/) 来辅助获取坐标点，请在图像处理软件中将图片大小缩放至100%，再把鼠标移动到图片上，获取指定位置坐标点的X，Y值，请按照左上，右上，右下，左下顺时针顺序创建矩形框四个坐标点序列。矩形框区域必须完全覆盖要识别的文字内容，请在不遮盖其他识别矩形区域的前提下，在识别区域四边留出足够的容错空间，从而获得准确的识别能力。在创建好模版后，首先请用模版原图片和模版ID进行文字识别测试，以确保模版能够提取您需要的信息。如果发现提取信息不完整的情况，请确认坐标点是否标注正确，并适当扩大识别区域重新创建模版。
...
| type | *String* |是|增加模版需要将*type*指定为*add*|
...
如果需要删除模版，可通过指定需要删除的模版 ID 进行删除。请注意，模版被删除后不能恢复。
...
在创建好模版后，可以通过对应的模版 ID 对图片进行文字识别，返回值为模版中识别区域名称与文本内容。

...
|    **名称**   | **描述**    |
|--------------|------------|
|通用文字识别（简体中文）|基于人工智能OCR（光学字符识别）技术可自动完成图片中文字的识别，并返回文字在图片中的坐标位置、置信度等信息。</br>支持简体中文。|
|通用文字识别（繁体中文）|基于人工智能OCR技术可自动完成图片中文字的识别，并返回文字在图片中的坐标位置、置信度等信息。</br>支持繁体中文。|
|自定义模板文字识别|您可以创建结构化模板，通过自助服务模板进行自动识别并输出结构化结果。|
|车牌信息识别|识别中华人民共和国境内常见机动车号牌信息。|

### 人脸与人体
|    **名称**   | **描述**    |
|--------------|------------|
|人脸识别|检测和定位图片或视频流中的人脸，并返回高精度的人脸框坐标信息。|
|人脸相似度比对|对图片中人脸面部特征进行高精准提取分析，从而获取两张人脸的相似度。|
|人体结构化属性|检测图片中人体的属性分析，例如性别、年龄。|
|智能人像分割|识别图像中的人体轮廓，与背景进行分离。|

### 图像理解
|    **名称**   | **描述**    |
|--------------|------------|
|图像相似度|比较两幅图片是否相似。|
|通用物体识别|检测图像中的对象和场景，并返回它们以及每个对象和场景的置信度。|
|色情内容审核|对图片进行审核，识别并获取色情量化程度信息。|

### 视觉内容生产
|    **名称**   | **描述**    |
|--------------|------------|
|图像超分辨率|将单幅图片进行智能放大，从而获得清晰度高、细节丰富的图像。|

### 自然语言理解
|    **名称**   | **描述**    |
|--------------|------------|
|文本相似度|评估两个单词、句子、段落或文档的相似性。对两个文本的相似或不同程度进行评分。|


## 快速部署

这个代码库包含了完整的用 Typescript 语言编写的 AWS CDK 工程，如果您想使用上述深度学习功能而不在本地重新编译整个工程源代码，您可以选择使用 AWS CloudFormation 模板来快速部署，编译好的 CloudFormation 模板可通过如下链接获取：https://aws-gcr-solutions.s3.amazonaws.com/Aws-gcr-ai-solution-kit/v1.2.0/AI-Solution-Kit.template

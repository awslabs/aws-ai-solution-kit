...
os.environ['ACCESS_POINT'] = BASE_DIR + '/model'
access_point = os.environ['ACCESS_POINT']
ort_session = onnxruntime.InferenceSession(access_point + '/humanseg_720.onnx')
...
def lambda_handler(event, context):
    ...
    img = transform.resize(src, (new_h, new_w), mode='constant')
    img = img / np.max(img)
...
event = {
    "body": {
        "url": "https://aip.bdstatic.com/portal-pc-node/dist/1628160386339/images/technology/body/seg/3.jpg"
    }
}

# %%

ResultMask = json.loads(lambda_handler(event, context='')['body'])['result']
print(Base64Image.from_base64_image_string(ResultMask).get_pil_image())
